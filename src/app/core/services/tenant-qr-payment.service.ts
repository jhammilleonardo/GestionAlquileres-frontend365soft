...
import { QrPayment, QrPaymentStatus, GenerateQrPaymentDto, VerifyQrDto } from '../models/payment.model';
...
export class TenantQrPaymentService {
    ...
    loadQrList(): void {
        ...
        this.http.get<QrPayment[]>(this.buildUrl())
            .pipe(
                tap(list => {
    ...
    verifyQr(dto: VerifyQrDto): Observable<QrPayment> {
        return this.http.post<QrPayment>(this.buildUrl('/verificar'), dto).pipe(
            tap(qr => {
                ...
                this.qrListSignal.update(list =>
                    list.map(q => (q.id === qr.id ? qr : q))
                );
    ...
    isTerminalStatus(status: QrPaymentStatus): boolean {
        return status === QrPaymentStatus.PAGADO
