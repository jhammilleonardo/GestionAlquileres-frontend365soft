...
export class AdminUserService {
    ...
    loadUsers(filters?: UserFilters): void {
        ...
        this.apiHttp.get<User[]>(endpoint, params, this.headers)
            .pipe(
                tap(users => {
    ...
    loadTenants(): void {
        ...
        this.apiHttp.get<User[]>(endpoint, {}, this.headers)
            .pipe(
                tap(users => {
                    // Filtrar solo usuarios con rol USER o INQUILINO
                    const tenants = users.filter(
                        u => u.role === 'USER' || u.role === 'INQUILINO'
                    );
    ...
    getUser(id: number): Observable<User> {
        ...
        return this.apiHttp.get<User>(endpoint, {}, this.headers)
            .pipe(
                catchError(error => {
