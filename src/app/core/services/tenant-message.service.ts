...
import { Message, MessageThread, CreateMessageDto, ReplyMessageDto, MessageStatus } from '../models/message.model';
...
export class TenantMessageService {
    ...
    loadThreads(): void {
        ...
        this.http.get<MessageThread[]>(`${environment.apiUrl}/tenant/message-threads`)
            .pipe(
                tap(threads => {
    ...
    getMessage(id: number): Observable<Message> {
        return this.http.get<Message>(`${environment.apiUrl}/tenant/messages/${id}`)
            .pipe(
                tap(message => {
                    const parsedMessage = {
                        ...message,
    ...
    createMessage(message: CreateMessageDto): Observable<Message> {
        ...
        return this.http.post<Message>(`${environment.apiUrl}/tenant/messages`, message)
            .pipe(
                tap(newMessage => {
                    const parsedMessage = {
                        ...newMessage,
    ...
    replyMessage(reply: ReplyMessageDto): Observable<Message> {
        ...
        return this.http.post<Message>(`${environment.apiUrl}/tenant/messages/reply`, reply)
            .pipe(
                tap(newMessage => {
                    const parsedMessage = {
                        ...newMessage,
    ...
    markAsRead(messageId: number): Observable<void> {
        return this.http.put<void>(`${environment.apiUrl}/tenant/messages/${messageId}/read`, {})
            .pipe(
                tap(() => {
                    this.messagesSignal.update(messages =>
                        messages.map(m =>
                            m.id === messageId
                                ? { ...m, status: MessageStatus.READ, read_at: new Date() }
                                : m
                        )
                    );
                }),
    ...
    archiveMessage(messageId: number): Observable<void> {
        return this.http.put<void>(`${environment.apiUrl}/tenant/messages/${messageId}/archive`, {})
            .pipe(
                tap(() => {
                    this.messagesSignal.update(messages =>
                        messages.map(m =>
