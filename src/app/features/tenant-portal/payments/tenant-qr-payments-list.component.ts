...
import {
    LucideAngularModule,
    QrCode, Plus, Clock, CheckCircle2, XCircle, AlertCircle,
    RefreshCw, Banknote
} from 'lucide-angular';
...
import {
    QrPayment, QrPaymentStatus,
    QrPaymentStatusLabels, QrPaymentStatusColors,
    PaymentTypeLabels, CurrencySymbols, Currency, PaymentType
...
<div class="qrl-container">
    ...
    @else {
        <mat-card class="list-card">
            <div class="list-header">
                ...
                <button mat-icon-button (click)="qrService.loadQrList()"
                        [disabled]="qrService.isLoading()" aria-label="Recargar">
                            ...
                            @switch (qr.status) {
                                @case (QrStatus.PAGADO) {
                                    <lucide-icon [img]="CheckCircle2" [size]="20"
                                                 style="color:#10b981"></lucide-icon>
                                }
                                @case (QrStatus.EXPIRADO) {
                                    <lucide-icon [img]="Clock" [size]="20"
                                                 style="color:#64748b"></lucide-icon>
                                }
                                @case (QrStatus.CANCELADO) {
                                    <lucide-icon [img]="XCircle" [size]="20"
                                                 style="color:#ef4444"></lucide-icon>
                                }
                                @default {
                                    <lucide-icon [img]="QrCode" [size]="20"
                                                 style="color:#f59e0b"></lucide-icon>
                                }
                            }
                        </div>
    ...
    styles: [`
        ...
        .empty-card h2 { color: #1e293b; margin: 0 0 8px; }
        .empty-card p { color: #64748b; margin: 0 0 24px; }
        ...
        .list-count { font-size: 0.8rem; color: #94a3b8; font-weight: 600; }
        ...
        .qr-row:hover { background: #f8fafc; }
        ...
        @media (max-width: 600px) {
            ...
            .qr-row { flex-wrap: wrap; gap: 10px; }
            .qr-info { min-width: 100%; order: 2; }
