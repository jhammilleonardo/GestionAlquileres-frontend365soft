...
import {
    LucideAngularModule,
    ArrowLeft, QrCode, CheckCircle2, AlertCircle,
    RefreshCw, XCircle, Clock, Banknote, Info, Download
} from 'lucide-angular';
...
import {
    QrPayment, QrPaymentStatus, QrPaymentStatusLabels, QrPaymentStatusColors,
    PaymentType, PaymentTypeLabels, Currency, CurrencyLabels, CurrencySymbols
...
<div class="qr-container">
    ...
    @if (!qrService.activeQr()) {
        <mat-card class="form-card">
            ...
            <div class="info-banner">
                ...
                <span>
                    El código QR es compatible con <strong>MC4 / SIP Bolivia</strong>.
                    Puedes escanerlo desde cualquier app bancaria que soporte QR de
                    pago interbancario.
                </span>
            </div>
            ...
            <form [formGroup]="form" (ngSubmit)="onGenerate()" class="qr-form">
                ...
                <mat-form-field appearance="outline" class="full-width">
                    ...
                    <input matInput formControlName="notes" maxlength="200"
    ...
    @if (qrService.activeQr(); as qr) {
        ...
        @if (qr.status === QrStatus.PAGADO) {
            <mat-card class="result-card success">
                ...
                <p>Tu pago por <strong>{{ currencySymbol(qr.currency) }}{{ qr.amount | number:'1.2-2' }}</strong>
        ...
        @if (qr.status === QrStatus.PENDIENTE) {
            <mat-card class="qr-display-card">
                ...
                <!-- Instrucciones -->
                <div class="qr-steps">
                    <div class="step"><span class="step-no">1</span>Abre tu app bancaria o billetera digital</div>
                    <div class="step"><span class="step-no">2</span>Selecciona la opción <strong>Pagar con QR</strong></div>
                    <div class="step"><span class="step-no">3</span>Escanea el código y confirma el pago</div>
                    <div class="step"><span class="step-no">4</span>Esta pantalla se actualizará automáticamente</div>
                </div>
                ...
                @if (qr.expires_at) {
                    <div class="expires-row">
                        ...
                        <span class="poll-label">
                            Verificando cada {{ pollIntervalSec }}s
                        </span>
                    </div>
                }
                ...
                <div class="qr-actions">
                    <button mat-stroked-button (click)="manualVerify(qr)"
                            [disabled]="polling()">
                        @if (polling()) { <mat-spinner diameter="16"></mat-spinner> }
                    ...
                    <button mat-stroked-button color="warn" (click)="onCancel(qr)"
                            [disabled]="cancelling()">
                        @if (cancelling()) { <mat-spinner diameter="16"></mat-spinner> }
    ...
    styles: [`
        ...
        .icon-primary { color: var(--mat-sys-primary, #1976d2); }
        ...
        .result-card.expired .result-icon { color: #64748b; }
        .result-card.cancelled .result-icon { color: #ef4444; }
        ...
        @media (max-width: 600px) {
            ...
            .qr-display-header { flex-direction: column; align-items: flex-start; gap: 12px; }
            .form-actions { flex-direction: column-reverse; }
            .form-actions button { width: 100%; justify-content: center; }
            .qr-actions { flex-direction: column; }
            .qr-actions button { width: 100%; justify-content: center; }
...
export class TenantQrGenerateComponent implements OnInit, OnDestroy {
    ...
    readonly Info        = Info;    readonly Download     = Download;
    // ── Services ──────────────────────────────────────────────────────
    ...
    ngOnInit(): void {
        ...
        const tryPrefill = () => {
            ...
            if (c) {
                const amount = typeof c.monthly_rent === 'number'
                    ? c.monthly_rent
